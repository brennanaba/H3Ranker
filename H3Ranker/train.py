    ...
    for l in lens:
        ...
        for j in range(len(df)//batch + 1):
            ...
            for i in range(len(structs)):
                ...
                first = pair[0] 
                first = encode_distances(first, float(resol[i])/4)
                second = pair[1]
                second = encode_angles(second, float(resol[i])*angle_diff/(4*dist_diff))
                second1 = pair[2]
                second1 = encode_angles(second1, float(resol[i])*angle_diff/(4*dist_diff))
                second2 = pair[3]
                second2 = encode_angles(second2, float(resol[i])*angle_diff/(4*dist_diff))
