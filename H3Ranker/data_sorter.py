...
confs = pd.read_csv(os.path.join(current_directory,"multiple_conformations.csv"))

# Separating the test data from the train and validate data. 

new_test = []
for v in test_set:
    ch1 = [y[:-1] for y in confs[[v == x[:-1] for x in confs.Chain2]].Chain1.values]
    ch2 = [y[:-1] for y in confs[[v == x[:-1] for x in confs.Chain1]].Chain2.values]
    new_test += ch1 + ch2 + [v]
...
new_vals = []
for v in val_set:
    ch1 = list(confs[v == confs.Chain2].Chain1.values)
    ch2 = list(confs[v == confs.Chain1].Chain2.values)
    new_vals += ch1 + ch2 + [v]
